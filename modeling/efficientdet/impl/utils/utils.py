...
def standard_to_bgr(list_color_name):
    standard = []
    for i in range(
        len(list_color_name) - 36
    ):  # -36 used to match the len(obj_list)
...
def plot_one_box(
    img, coord, label=None, score=None, color=None, line_thickness=None
):
    tl = line_thickness or int(
        round(0.001 * max(img.shape[0:2]))
    )  # line thickness
    color = color
    ...
    if label:
        ...
        t_size = cv2.getTextSize(
            label, 0, fontScale=float(tl) / 3, thickness=tf
        )[0]
